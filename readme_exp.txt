...
Ci sono due branch su git, master e attempt
Attempt ha miseramente fallito dei sinceri tentativi di miglioramento.

1)	Primo tra tutti ha implementata una gestione pulita dell'I/O (o meglio presunta pulita) in cui tutti i parametri di input sono generati da pre_Experimentum_united
	nella cartella madre di ogni presa dati e poi ogni esecuzione del programma lascia traccia di questa configurazione salvandola esattamente come è nella cartella di 
	analisi associata.
2) 	Avevo provato a migliorare la riproduzione del tempo con il serpentine range supportato anche dalla trasposizion della matrice che viene fuori dal 
	programma di Claudia, ma in realtà questo tentativo non sta riuscendo

3)	Ho provato a gestire più cose con i dizionari, esempio i p0 di riferimento per la tipologia normal o almost (ah, su attempt brillouin_higher e highest, 
	giustamente, non ci sono più in quanto non hanno senso alcuno)
...
Ho provato a fare una versione definitiva di Exp e lib in Brillouin e lib_Brillouin (su attempt), ma ho rinunciato


Fase 0) PREPARAZIONE AL LANCIO

Si utilizza un altro programma: pre_Experimentum_united.py:
	la sua versione migliore sta sulla branch attempt
...
Fase 1) ACQUISIZIONE E MODIFICA SPETTRI
	...
	- Il salvataggio di queste posizioni che sono quelle usate per il fit è un po' controverso, su master viene fatto con i .txt ma sicuramente ci sono problemi	
		nell'I/O che su attempt sono risolti pagando un enorme allungamento dei tempi
	

Fase 2) FIT MARKOVIANO
	 
	procedo con il fit modello markoviano ( perchè so che viene bene e potrà poi essere il punto di partenza per quell'altro che comunque non è mai venuto bene)
...
Fase 3) Fit totale
	...
	- viene svolto per ogni elemento a partire dai parametri markoviani, aggiungendo tau = 1. Delta = Gamma
		ma non viene per niente, non è mai venuto
		in particolare: tende a aumentare errore su omega (anche se la stima rimane buona), gamma diventa orribile, tau fa schifo proprio, i valori per cui viene		
		significativo (ossia dell'ordine di 1/Omega o minore) non esistono e se esistono sono errori

CONSIGLIO FINALE e osservazioni

	CONSIGLIO PER IL FUTURO questa cosa delle condizioni iniziali è ESTREMAMENTE IMPORTANTE: per esempio usando il metodo levenberg-marquadrt che sembra dare risultati
	migliori e più veloci, sono fondamentali in quanto non abbiamo bounds
	questa cosa del passaggio sembra essere la principale causa del fallimento di attempt, in quanto errori di un certo tipo si propagano inevitabilmente
	
	- I BOUNDS anche nel caso in cui usati sono fondamentali, sceglierli male è successo che ha portato a esplosioni di alcuni parametri compromettendo il 
		jacobiano e di conseguenza la matrice di covarianza da cui calcolo gli errori 
	
	metodo lm tende a far esplodere la gaussiana

	INSOMMA MOLTI DEI PROBLEMI SONO LEGATI ALLA GRANDE MOLE DI DATI, LA COSA MIGLIORE SEMBRA EFFETTUARE UNA PARALLELIZZAZIONE ARTIGIANALE O PRECISA IN BASE
	AL NUMERO DI CORE DISPONIBILI:
	FACCIO IL FIT SU SOTTOINSIEMI DI MATRICI, LIMITANDO PROPAGAZIONE DI errori e probabilmente anche velocizzando di molto il tutto
