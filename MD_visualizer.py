...
# O5
ref_mean_O5 = np.mean(DF.Covariance_Mean_Prot_BS_O5[DF.identifier == 'wtc7_16'][DF.Is_BS_O5 == True][DF.Is_Prot == True])
ref_std_O5  = np.std(DF.Covariance_Mean_Prot_BS_O5[DF.identifier == 'wtc7_16'][DF.Is_BS_O5 == True][DF.Is_Prot == True])
...
# P
ref_mean_P = np.mean(DF.Covariance_Mean_Prot_BS_P[DF.identifier == 'wtc7_16'][DF.Is_BS_P == True][DF.Is_Prot == True])
ref_std_P = np.std(DF.Covariance_Mean_Prot_BS_P[DF.identifier == 'wtc7_16'][DF.Is_BS_P == True][DF.Is_Prot == True])
...
normalizers_O5 = [np.mean(DF.RMSF[DF.identifier == key][DF.Is_Prot == True][DF.Is_BS_O5==True])  for key in now_keys]
...
for x,x_err, y, key in zip(x_vals, x_errs, correlators_O5, now_keys):
    ax.errorbar(x, y, fmt = 'o', ecolor = 'orange', mew = 0.1, color = 'black' if key in wtc_keys else 'green' if key in cov_keys else 'goldenrod', label = 'wtc_series' if key == wtc_keys[0] else 'cov_series' if key == cov_keys[1] else 'mtc series' if key == mtc_keys[0] else None)
...
for x,x_err, y, key, norm in zip(x_vals, x_errs, correlators_O5, now_keys, normalizers_O5):
    ax.errorbar(x, y/norm, fmt = 'o', ecolor = 'orange', mew = 0.1, color = 'black' if key in wtc_keys else 'green' if key in cov_keys else 'goldenrod', label = 'wtc_series' if key == wtc_keys[0] else 'cov_series' if key == cov_keys[1] else 'mtc series' if key == mtc_keys[0] else None)
    plt.annotate('NMR' if key == 'wtc1' else 'wtc1' if key == 'wtc1_h' else 'wtc1_new' if key == 'wtc1_h_new' else key, (x,y/norm), xytext = (5, 10), textcoords = 'offset points')
...
normalizers_P = [np.mean(DF.RMSF[DF.identifier == key][DF.Is_Prot == True][DF.Is_BS_P==True])  for key in now_keys]
...
for x,x_err, y, key, norm in zip(x_vals, x_errs, correlators_P, now_keys, normalizers_P):
    ax.errorbar(x, y/norm, fmt = 'o', ecolor = 'orange', mew = 0.1, color = 'black' if key in wtc_keys else 'green' if key in cov_keys else 'goldenrod', label = 'wtc_series' if key == wtc_keys[0] else 'cov_series' if key == cov_keys[1] else 'mtc series' if key == mtc_keys[0] else None)
    plt.annotate('NMR' if key == 'wtc1' else 'wtc1' if key == 'wtc1_h' else 'wtc1_new' if key == 'wtc1_h_new' else key, (x,y/norm), xytext = (5, 10), textcoords = 'offset points')
