...
non_fitted, fitted, exceded = Unpack_Fit(fit)

Fit_Map     =   Get_Fit_Map(n_rows, n_cols, non_fitted, exceded, excluded, fig = 'Markov_Fit_Map', path = now_path)
Omega_Map   =   Get_Parameter_Map('Omega', matrix, n_rows, n_cols, fitted, exceded, excluded ,fig = 'Markov_Omega_Map', path = now_path)
Gamma_Map   =   Get_Parameter_Map('Gamma', matrix, n_rows, n_cols, fitted, exceded, excluded ,fig = 'Markov_Gamma_Map', path = now_path)
"""
#%%
# 5)fit completo
...
#6) after fit
too  = ()
non_fitted_tot, fitted_tot, exceded_tot =   Unpack_Fit(fit_tot)
too    =   too + (Whose_Gamma_Too_High(1., matrix, fitted, exceded_tot),)
excluded += too
Fit_Map     =   Get_Fit_Map(n_rows, n_cols, non_fitted_tot, exceded_tot, excluded, fig = 'Total_Fit_Map', path = now_path)
Omega_Map   =   Get_Parameter_Map('Omega', matrix, n_rows, n_cols, fitted_tot, exceded_tot, excluded ,fig = 'Omega_Map', path = now_path)
Gamma_Map   =   Get_Parameter_Map('Gamma', matrix, n_rows, n_cols, fitted_tot, exceded_tot, excluded ,fig = 'Gamma_Map', path = now_path)

Save_Params(now_path, n_rows, n_cols, matrix, fitted_tot+exceded_tot)
Save_Fit_Info(now_path, n_rows, n_cols, fit_tot)

#%%
#pre fitted cambia fitted in boni
#VERIFICHE DA QUI IN POI
""" 
#ESCLUSIONE A MANO
to_add  = Whose_Gamma_Too_High(5., matrix, fitted, exceded)
...
for (ii,jj) in fitted[40:60]:
    plt.figure()
    plt.plot(matrix[ii][jj].x_freq, matrix[ii][jj].y)
    plt.plot(matrix[ii][jj].x_freq, matrix[ii][jj].y_fit)
    plt.title(str((ii,jj)))
